...
**验收标准**:
- [ ] Transaction Card 显示缩略图（本地优先，fallback S3）
- [ ] 排序：发票日期/处理时间 × 升序/降序
- [ ] 分页：每页 20 条，底部导航
- [ ] 软删除：本地标记 status='deleted'，UI 隐藏
- [ ] 测试通过
...
| 功能 | 原计划 | 简化方案 | 延迟到 |
|------|--------|----------|--------|
| Lightbox | 完整组件 | 简单 modal 或跳过 | MVP5 |
| S3 图片 | presign URL | 先只显示本地图片 | MVP5 |
| dirty_sync | 新列 | 用现有 status 列 | MVP5 |
| Confirm 同步 | 立即推送 | 本地生效即可 | MVP5 |
| URL query param | 记录分页状态 | 内存状态 | 可选 |

**核心交付** (MVP4):
1. ✅ 排序 + 分页（必须）
2. ✅ 软删除（必须）
3. ✅ 本地图片缩略图（如有）
4. ⏸️ S3 图片、Lightbox、云端同步 → MVP5
