/**
 * Instant Processor Lambda (Phase 4 - P2)
 *
 * S3-triggered Lambda that processes receipt images:
 * - Multi-model AI OCR (Azure DI, Bedrock Nova) with fallback
 * - Merchant list matching for standardization
 * - TraceId recovery from S3 metadata (Pillar N)
 * - Idempotent transaction creation (Pillar Q)
 * - Schema validation with graceful fallback (Pillar B)
 * - S3 lifecycle management (uploads/ → processed/)
 * - Guest user TTL tracking
 */
...
import {
  ...
  TransactionSchema,
  SystemConfigSchema,
} from '/opt/nodejs/shared/schemas.mjs';
import {
  analyzeAzureDI,
  convertModelResultToOcrResult,
} from '/opt/nodejs/shared/model-analyzer.mjs';
import { getAzureCredentials } from '/opt/nodejs/shared/azure-credentials.mjs';
import type { S3Event } from './types/aws-events.js';
...
let cachedMerchantList: string[] | null = null;

/**
 * Load merchant list from S3 (cached in memory after first load)
 */
async function loadMerchantList(): Promise<string[]> {
  if (cachedMerchantList) {
    logger.debug(EVENTS.MERCHANT_LIST_CACHE_HIT, {
      count: cachedMerchantList.length,
    });
  ...
  try {
    ...
    if (!body) {
      logger.warn('MERCHANT_LIST_LOAD_FAILED', { reason: 'Empty response body' });
      return [];
    }

    const data = JSON.parse(body) as { merchants: Array<{ name: string }> };
    ...
    logger.warn('MERCHANT_LIST_LOAD_FAILED', { error: errorMessage });
    return [];
  }
}
 ...
 */
async function recoverTraceIdFromS3(
  bucket: string,
  key: string
): Promise<string | null> {
  try {
...
export async function handler(event: S3Event): Promise<void> {
  ...
  for (const record of event.Records) {
    ...
    if (keyParts.length < 3) {
      logger.warn(EVENTS.IMAGE_PROCESSING_FAILED, {
        key,
        reason: 'INVALID_KEY_STRUCTURE',
      });
      continue;
    }
    ...
    try {
      ...
      const imageExt = fileName.split('.').pop()?.toLowerCase() || 'jpg';
      ...
      logger.debug('IMAGE_FORMAT_DETECTED', {
        fileName,
        extension: imageExt,
        format: imageFormat,
      });
      ...
      try {
        ...
        if (configResult.Item) {
          ...
          const primaryModel =
            systemConfig.primaryModelId || systemConfig.modelId || modelId;
          ...
          if (primaryModel === 'azure_di') {
            ...
            if (secretArn) {
              ...
              throw new Error(
                'Azure DI selected but no secret ARN configured'
              );
            }
          } else {
      ...
      if (useAzureAsPrimary) {
        ...
        try {
          ...
          try {
            ...
            const fallbackOcrText =
              fallbackResponseBody.output?.message?.content?.[0]?.text || '';
            ...
            logger.error('FALLBACK_FAILED_SKIPPING_RECORD', {
              imageId,
              primaryError: azureErrorMessage,
        ...
        try {
          ...
          logger.error(EVENTS.AIRLOCK_BREACH, {
            userId,
            imageId,
      ...
      if (!validationResult.success) {
        ...
        transaction = {
          ...
          description:
            parsed.description || 'AI extraction incomplete - please verify',
      ...
      try {
        ...
        const headErrMessage =
          headErr instanceof Error ? headErr.message : 'Unknown error';
        logger.error(EVENTS.IMAGE_PROCESSING_FAILED, {
          userId,
          imageId,
          reason: 'S3_COPY_VERIFICATION_FAILED',
          processedKey,
          error: headErrMessage,
        });
      ...
      logger.info(EVENTS.IMAGE_PROCESSING_COMPLETED, {
        userId,
        imageId,
        transactionId,
        s3Key: processedKey,
