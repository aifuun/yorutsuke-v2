/**
 * Quota Lambda (Phase 4 - P2)
 *
 * Returns user's daily quota usage and remaining uploads:
 * - Tier-based limits (guest: 30, free: 50, basic: 100, pro: 300)
 * - JST (UTC+9) date handling for daily resets
 * - Guest user data expiration tracking (60 days TTL)
 * - TraceId propagation
 */
...
/**
 * Guest expiration info
 */
interface GuestExpiration {
  lastActiveAt: string;
...
async function updateLastActive(
  ...
  try {
    ...
    logger.warn(EVENTS.QUOTA_CHECK_FAILED, {
      step: 'update_last_active',
      error: errorMessage,
    });
 ...
 */
function getNextMidnightJST(): string {
  const now = new Date();
  // Current JST time
...
export async function handler(
  ...
  try {
    ...
    logger.error(EVENTS.QUOTA_CHECK_FAILED, {
      error: errorMessage,
      stack: errorStack,
    });
    return {
